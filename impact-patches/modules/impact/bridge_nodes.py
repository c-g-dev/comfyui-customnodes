...
class PreviewBridgeWithMaskFreeze:
    @classmethod
    def INPUT_TYPES(s):
        return {"required": {
                    "images": ("IMAGE",),
                    "image": ("STRING", {"default": ""}),
                    "mask_freeze": (["disabled", "enabled"], {"default": "disabled"}),
                    },
    ...
    def doit(self, images, image, mask_freeze, unique_id):
        need_refresh = False

        if unique_id not in core.preview_bridge_cache:
            need_refresh = True
        elif mask_freeze == 'enabled':
            need_refresh = False
        ...
        if not need_refresh:
            pixels, mask, path_item = PreviewBridgeWithMaskFreeze.load_image(image)
            image = [path_item]
        else:
